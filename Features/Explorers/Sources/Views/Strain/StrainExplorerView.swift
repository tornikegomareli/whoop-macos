...
public struct StrainExplorerView: View {
    ...
    public var body: some View {
        Group {
            switch model.state {
            case .idle, .loading:
                ProgressView("Preparing your cycle history…")
                    .controlSize(.large)
            case let .loaded(snapshot):
                StrainExplorerContentView(
                    snapshot: snapshot,
            ...
            case let .failed(message):
                ContentUnavailableView {
                    Label("Cycle history unavailable", systemImage: "bolt")
                } description: {
                    Text(message)
                } actions: {
